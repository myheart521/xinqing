...
import {sendAdminAIAgentMessage, sendAdminMCPAIAgentMessage, sendAIChatMessage} from "@/api/ai/chat.js";
// 全局SDK实例，与配置页面保持一致
...
export default {
            ...
            while (true) {
              ...
              if (done) {
                ...
                if (done && tempAiMessage.content) {
                  await digitalHumanStore.sendTextToAvatar(tempAiMessage.content);
                }
                // 流结束
  ...
  methods: {
      ...
      try {
        ...
        player
            ...
            .on(PlayerEvents.playNotAllowed, function () {
              console.log("播放被阻止：触发了浏览器限制自动播放策略，需要用户交互后调用player.resume()");
              // 自动恢复播放
              setTimeout(() => {
                try {
                  player.resume();
    ...
    async sendWelcomeMessage() {
      try {
        console.log('发送欢迎语...');
        const welcomeText = "您好！我是您的教学助手，我具备专业的心理咨询经验。";
