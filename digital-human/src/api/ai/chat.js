...
const STUDENT_AI_AGENT_API = '/qwen/stream/flux'; // 学生端智能体入口接口
/**
 * 发送联网搜索消息
 ...
 * @returns {Promise} 返回一个支持流式响应的Promise对象
 */
export const sendStudentAIAgentMessage = (prompt, memoryId) => {
  const params = {
    prompt,
    memoryId
  };
  ...
  console.log('管理端智能体 - 发送消息参数:', { prompt, memoryId });
  const requestUrl = myRequestUrl+STUDENT_AI_AGENT_API + '?' + new URLSearchParams(params).toString();
  console.log('管理端智能体 - 发送请求URL:', requestUrl);
