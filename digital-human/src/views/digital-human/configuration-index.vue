      ...
      <el-aside width="320px" class="control-panel">
        <div class="panel-header">
          ...
          <div class="mobile-nav" v-show="isMobile">
            <el-button size="small" @click="scrollToSection('config')">配置</el-button>
            <el-button size="small" @click="scrollToSection('control')">控制</el-button>
            <el-button size="small" @click="scrollToSection('test')">测试</el-button>
          </div>
        </div>
        ...
        <div class="control-section">
          ...
          <el-button @click="stopRecord()" v-if="recorderbutton" type="danger" class="control-btn">
            <el-icon>
              <Close />
            </el-icon>
            关闭录音
        ...
        <div class="control-section">
          ...
          <div class="button-group">
            <el-button @click="testSmartAction()" type="success" class="control-btn">
              <el-icon>
                <VideoPlay />
              </el-icon>
              测试智能动作
            </el-button>
            <el-button @click="batchTestActions()" type="info" class="control-btn">
              <el-icon>
                <Document />
              </el-icon>
              批量测试
      ...
      >
        <el-form :model="form" label-width="120px">
          ...
          <el-form-item label="Appid" required>
            <el-input v-model="form.appid" placeholder="请输入Appid" clearable />
          </el-form-item>

          <el-form-item label="ApiKey" required>
            <el-input v-model="form.apikey" placeholder="请输入ApiKey" clearable />
          </el-form-item>
          ...
          <el-form-item label="SceneId" required>
            <el-input v-model="form.sceneid" placeholder="请输入SceneId" clearable />
          </el-form-item>

          <el-form-item label="ServerUrl" required>
            <el-input v-model="form.serverurl" placeholder="请输入ServerUrl" clearable />
          </el-form-item>
        </el-form>
        ...
        <template #footer>
          <div class="dialog-footer">
            ...
            <el-button type="primary" @click="handleApiInfoConfirm"> 确 定 </el-button>
          </div>
        </template>
      </el-dialog>
      ...
      >
        ...
        <el-form :model="setglobalparamsform" label-width="140px">
          <el-form-item label="视频协议">
            <el-tooltip content="支持webrtc/xrtc/rtmp(控制台打印视频流地址)" placement="top">
          ...
          <el-form-item label="透明背景">
            <el-tooltip content="仅支持xrtc协议" placement="top">
              <el-icon class="help-icon">
                <QuestionFilled />
              </el-icon>
            </el-tooltip>
            <el-select
              v-model="setglobalparamsform.stream.alpha"
              placeholder="请选择透明度设置"
              class="form-select"
            >
        ...
        <template #footer>
          <div class="dialog-footer">
            ...
            <el-button type="primary" @click="handleGlobalParamsConfirm"> 确 定 </el-button>
          </div>
        </template>
      </el-dialog>
    </el-container>
...
import { testSmartAction as testSmartActionUtil, batchTestActions as batchTestActionsUtil } from '@/utils/actionTestUtils.js'
...
export default {
  ...
  methods: {
    ...
    writeText() {
      if (avatarPlatform2 != null) {
        ...
        if (text != '') {
          // 过滤文本内容
          const filteredText = filterForDigitalHuman(text)
          if (!filteredText) {
          ...
          if (this.vc !== '') {
            ttsConfig.tts.vcn = this.vc //变声
          }

          avatarPlatform2.writeText(filteredText, ttsConfig)
        } else {
          alert('内容不许为空')
        }
      } else {
    ...
    writeCmd() {
      if (avatarPlatform2 != null) {
        if (this.action) {
          ...
          this.open2(
            `执行动作: ${this.currentActionList.find((item) => item.id === this.action)?.name || this.action}`,
          )
        } else {
          this.$message({
            message: '请先选择要执行的动作',
    ...
    onAvatarChange(avatarId) {
      if (avatarId && this.actionData[avatarId]) {
        ...
        this.action = ''
        this.open2(
          `已加载 ${this.avatarList.find((avatar) => avatar.id === avatarId)?.name || avatarId} 的动作列表`,
        )
      } else {
    ...
    // 测试智能动作
    testSmartAction() {
      const avatarId = this.setglobalparamsform.avatar.avatar_id
      if (!avatarId) {
        this.$message.error('请先选择数字人形象')
        return
      }

      if (!this.testActionContent.trim()) {
        this.$message.error('请输入测试内容')
        return
      }

      const result = testSmartActionUtil(this.testActionContent, avatarId)

      if (result.smartActions.length > 0) {
        const action = result.smartActions[0]
        this.$message.success(`推荐动作: ${action.name}`)

        // 自动执行推荐的动作
        if (avatarPlatform2) {
          avatarPlatform2.writeCmd('action', action.id)
          this.open2(`执行动作: ${action.name}`)
        }
      } else {
        this.$message.warning('没有找到合适的动作')
      }
    },
    ...
    batchTestActions() {
      ...
      const results = batchTestActionsUtil(avatarId)
      console.log('批量测试结果:', results)

      this.$alert(
        `批量测试完成，共测试了 ${results.length} 个场景\n` +
        `详细结果请查看控制台`,
        '批量测试结果',
        {
          confirmButtonText: '确定',
    ...
    scrollToSection(section) {
      const sectionMap = {
        config: '.control-section:nth-child(2)', // 配置设置
        control: '.control-section:nth-child(3)', // 文本控制
        test: '.control-section:last-child' // 测试区域
      }
  ...
  mounted() {
    ...
    if (
      this.setglobalparamsform.avatar.avatar_id &&
      this.actionData[this.setglobalparamsform.avatar.avatar_id]
    ) {
