<script setup>
	import {
		computed,
		onMounted,
		ref,
		watch
	} from 'vue';
	import {
		onLoad
	} from '@dcloudio/uni-app'

	let startY, moveY, windowHeight = 500,
		platform;
	let timeDiff = 0;
	let touchending;
	const props = defineProps({
		top: {
			//距离顶部距离，单位px
			type: Number,
			default: 0
		},
		height: {
			type: String,
			default: ''
		},
		// 下拉刷新的触发阈值（单位px）
		threshold: {
			type: Number,
			default: 80
		}
	})
	watch(() => assets.example.invalid, (newValue, oldValue) => {
		console.log("top(单位px):", assets.example.invalid)
	})
	//下偏移量
	const pageDeviation = ref(0)
	//回弹过渡时间
	const pageTransition = ref(0)
	//进入刷新准备状态
	const refreshReady = ref(false)
	// 进入刷新状态
	const refreshing = ref(false)
	// 是否显示图标
	const showIcon = ref(false)
	const emits = defineEmits(['setEnableScroll', 'refresh'])
	onMounted(() => {
		uni.getSystemInfo({
			success: function(e) {
				console.log(e);
				platform = e.platform;
				windowHeight = e.windowHeight;
			}
		})

	})
	const isH5 = computed(() => {
		return process.env.VUE_APP_PLATFORM === 'h5';
	})


	const pageTouchstart = (e) => {
		touchending = false;
		pageTransition.value = 0;
		startY = e.touches[0].pageY;
	}

	const pageTouchend = () => {
		touchending = true;
		if (moveY === 0) {
			showIcon.value = false;
			return;
		}
		pageTransition.value = 0.3;
		if (moveY >= props.threshold) {
			startPulldownRefresh();
		} else {
			pageDeviation.value = 0;
			showIcon.value = false;
		}

		if (refreshReady.value === true) {
			refreshReady.value = false;
		}
		//修复下拉一点回弹后页面无法滚动的bug
		emits('setEnableScroll', true);
		startY = moveY = 0;
	}

	const pageTouchmove = (e) => {
		if (touchending) {
			return;
		}
		moveY = (e.touches[0].pageY - startY) * 0.4;
		if (moveY >= 0) {
			showIcon.value = true;
			pageDeviation.value = moveY;
			emits('setEnableScroll', false)
		}
		if (moveY >= props.threshold && refreshReady.value === false) {
			refreshReady.value = true;
		} else if (moveY < props.threshold && refreshReady.value === true) {
			refreshReady.value = false;
		}
		if (platform === 'ios' && e.touches[0].pageY > windowHeight + 10) {
			pageTouchend();
		}
	}
	//开启下拉刷新
	const startPulldownRefresh = () => {
		if (+new Date() - timeDiff < 100) {
			return;
		}
		timeDiff = +new Date();
		refreshing.value = true;
		showIcon.value = true;
		pageDeviation.value = 40
		emits('refresh');
	}
	//结束下拉刷新
	const endPulldownRefresh = () => {
		refreshing.value = false;
		showIcon.value = false;
		pageDeviation.value = 0;
		emits('setEnableScroll', true);
	}

	defineExpose({
		endPulldownRefresh
	});
</script>

<template>
	<view class="refresh-content" :style="{
	       transform: 'translateY('+ pageDeviation +'px)',
	       transition: pageTransition + 's',
	       
	       maxHeight: isH5 ? 'calc(100% - ' + top + 'px)' : 'calc(100vh - ' + top + 'px)'
	   }" @touchstart="pageTouchstart" @touchmove="pageTouchmove" @touchend="pageTouchend">
		<!-- 下拉刷新 -->
		<view class="loading-wrapper">
			<image class="loading-icon" 
				:class="{active: refreshing, ready: refreshReady}"
				v-if="showIcon || refreshing"
				src="data:image/png;base64,">
			</image>
		</view>
		<slot></slot>
	</view>
</template>



<style scoped>
	.refresh-content {
		display: flex;
		flex-direction: column;
		position: relative;
	}

	/* 下拉刷新部分 */
	.loading-wrapper {
		position: absolute;
		left: 0;
		top: 0;
		transform: translateY(-100%);
		display: flex;
		justify-content: center;
		align-items: center;
		width: 100%;
	}

	.loading-icon {
		width: 70rpx;
		height: 70rpx;
		transition: .3s;
		z-index: -1;
	}

	.loading-icon.ready {
		transform: scaleX(1.3);
	}

	.loading-icon.active {
		animation: loading .5s ease-in infinite both alternate;
	}

	@keyframes loading {
		0% {
			transform: translateY(-20rpx) scaleX(1);
		}

		100% {
			transform: translateY(4rpx) scaleX(1.3);
		}
	}
</style>